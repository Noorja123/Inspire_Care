...
export function getGoogleCalendarUrl(details: AppointmentCalendarDetails): string {
  ...
  const dates = formatDateTimeLocal(details.date, details.time, 10);
...
export function getIcsFileContent(details: AppointmentCalendarDetails): string {
  ...
  const end = new Date(start.getTime() + 10 * 60000); // Default to 10 min duration
 ...
 */
function formatDateTimeLocal(dateStr: string, timeStr: string, durationMinutes = 10): string {
  const pad = (n: number) => n.toString().padStart(2, '0');
