...
SELECT
  ...
  (SELECT COUNT(DISTINCT COALESCE(patient_email, patient_phone)) FROM appointments WHERE deleted_at IS NULL) AS total_patients,
  (SELECT COUNT(*) FROM appointments WHERE appointment_date >= DATE_TRUNC('month', CURRENT_DATE) AND deleted_at IS NULL) AS appointments_this_month,
  (SELECT COUNT(*) FROM appointments WHERE appointment_date >= DATE_TRUNC('week', CURRENT_DATE) AND deleted_at IS NULL) AS appointments_this_week,
